...
class InfoFrame(ttk.Frame):
    ...
    def _draw_separator_frame(self, master: Any) -> ttk.Frame:
        ...
        ttk.Separator(frame, orient='horizontal').place(width=12, rely=0.44, x=-12)
        return frame
    
    def draw_tracker_frame(self, master: Any) -> None:
        """Отрисовка виджетов управления Трекером заказов"""
        HeaderLabel(master, text='Трекер заказов', anchor='n').pack(fill=ttkc.X, pady=(0, 3))
    ...
    def draw_status_line(self, master: Any) -> None:
        ...
        ttk.Label(master, text='Статус:', style='minipadding.TLabel').pack(anchor=ttkc.W, pady=(0, 3))
        frame = self._draw_separator_frame(master)
        status_var = ttk.StringVar(frame, value='Повтор в 11:25')
        ttk.Label(frame, text='Повтор в 11:25', style='minipadding.TLabel').pack()
