def product_test(product_class):
    ...
    for obj in obj_lst:     # Проверяем на присутсвие атрибутов
        ...
        print(obj)
        # Общие тесты
        assert 'full_name' in obj.__dict__, f'Отсутствует строка для указания полного имени в {name}'
        assert obj.full_name == '', f'Отсутствует значение по умолчанию для full_name в {name}'
        assert 'segment' in obj.__dict__, f'Отсутствует указание сегмента продукции в {name}'
        assert obj.segment != (), f'Отсутствует значение по умолчанию для segment в {name}'
        assert 'short_name' in obj.__dict__, f'Отсутствует список коротких имен в {name}'
        ...
        if name in ('Photobook', 'Layflat', 'Album', 'Journal', 'Photofolder', 'Canvas'):
            assert 'product_format' in obj.__dict__, f'Отсутствует указание формата продукта в {name}'
            assert obj.product_format != (), f'Отсутствует значение по умолчанию для product_format в {name}'
        # Утолщение
        if name in ('Photobook', 'Layflat'):
            assert 'book_option' in obj.__dict__, f'Отсутствует указание утолщения продукта в {name}'
        ...
        if name in ('Photobook', 'Layflat', 'Album', 'Photofolder'):
            assert 'lamination' in obj.__dict__, f'Отсутствует указание ламинации в {name}'
            assert obj.lamination != (), f'Отсутствует значение по умолчанию для lamination в {name}'
        # Тип оболожки
        if name in ('Photobook', 'Layflat', 'Album'):
            assert 'cover_type' in obj.__dict__, f'Отсутствует указание списка типов обложки в {name}'
            assert obj.cover_type != (), f'Отсутствует значение по умолчанию для cover_type в {name}'
        # Размер Картонки
        if name in ('Photobook', 'Layflat', 'Album', 'Photofolder'):
            assert 'carton_length' in obj.__dict__, f'Отсутствует указание длинны картона в {name}'
            assert obj.carton_length == 0, f'Отсутствует значение по умолчанию для carton_length в {name}'
            assert 'carton_height' in obj.__dict__, f'Отсутствует указание высоты картона в {name}'
            assert obj.carton_height == 0, f'Отсутствует значение по умолчанию для carton_height в {name}'
        # Клапан и шарнир
        if name in ('Photobook', 'Layflat', 'Album'):
            assert 'cover_clapan' in obj.__dict__, f'Отсутствует указание значений клапана в {name}'
            assert obj.cover_clapan != (), f'Отсутствует значение по умолчанию для cover_clapan в {name}'
            assert 'cover_joint' in obj.__dict__, f'Отсутствует указание значений шарнира в {name}'
            assert obj.cover_joint != (), f'Отсутствует значение по умолчанию для cover_joint в {name}'
        # Печатный материал обложек
        assert 'cover_print_mat' in obj.__dict__, f'Отсутствует cписок печатного материала обложек в {name}'
        ...
        if name in ('Photobook', 'Layflat', 'Album', 'Journal'):
            assert 'page_print_mat' in obj.__dict__, f'Отсутствует печатного материала разворота в {name}'
