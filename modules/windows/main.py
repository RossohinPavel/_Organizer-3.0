...
class MainWindow(tk.Tk):
    ...
    def show_processing_buttons(self, frame):
        ...
        btn2 = MyButton(master=container, text='Раскодировка', command=lambda: PageDecoder(master=self), width=15)
    ...
    def show_add_btn_menu(self):
        ...
        add_menu.add_command(label='Направляющие', command=lambda: CoverGuideLiner(master=self))
        add_menu.add_command(label='Разместить по каналам', command=lambda: PlacementByChannels(master=self))
        add_menu.add_command(label='Холсты', command=lambda: CanvasHandler(master=self))
        add_menu.add_separator()
        add_menu.add_command(label='Роддом', command=lambda: RoddomWindow(master=self))
        add_menu.add_separator()
        add_menu.add_command(label='Бакап', command=lambda: BackUpHandler(master=self))
        add_menu.add_command(label='Замена', command=lambda: ImageReplacement(master=self))
