__all__ = ('full_name', 'segment', 'short_name', 'size', 'book_option',
           'lamination', 'cover_type', 'carton_length', 'carton_height',
           'cover_clapan', 'cover_joint', 'cover_print_mat', 'page_print_mat',
           'cover_canal', 'page_canal', 'dc_break', 'dc_overlap', 'dc_top_indent',
           'dc_left_indent')


def product_property(func):
    """
    Декоратор, в который необходимо обернуть свойство (func).
    В качестве имени свойства (атрибута, который присоединится к объекту) будет использовано имя функции.
    В качестве значения свойства будет None. Если default_values == True, то вместо None будет установлено то, что вернет эта функция  
    param func: ссылка на функцию.
    """
    def wrapper(obj, default_values):
        setattr(obj, func.__name__, func(obj.category()) if default_values else None)
    wrapper.__name__, wrapper.__doc__ = f'Product property <{func.__name__}>', func.__doc__
    return wrapper    
...
def size(obj_name):
    ...
    formats = {'Photobook': bf[:-1], 'Layflat': bf[4:9], 'Album': bf[4:], 'Journal': bf[4:6], 'Photofolder': bf[5:7], 'Canvas': canvas}
...
def cover_canal(obj_name):
    ...
    return ('160', '161', '162', '163', '164', '165', '166', '204', '205', '214', '240', '242', '243', '245', '266', '36', 'POLI')
